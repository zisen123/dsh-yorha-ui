...
/** Owner props a brand-mark slot (hero / sidebar) hands to its occupant. */
interface BrandMarkProps {
  size?: number;
  className?: string;
}

/** The slice of the React seat this plugin uses (platform seed module). */
type ReactProps = { className?: string; dangerouslySetInnerHTML?: { __html: string } };

/** Minimal structural view of the client-side slots service. */
interface SlotsService {
  /**
   * Register a component into a slot while the calling entry is alive.
   * @param name - slot key declared by the owning factory.
   * @param options - registration options (`name` + optional `id`).
   * @param component - render function receiving the slot owner's props.
   * @returns disposer unregistering the component.
   */
  register(
    options: { name: string; id?: string },
    component: (props?: BrandMarkProps) => unknown
  ): () => void;
  /**
   * Attach registrations to the calling entry's lifecycle.
   * @param _name - slot key the injection targets.
   * @param register - callback performing the registration; its disposer is wired to the entry.
   */
...
export function apply(ctx: ClientCtx): void {
  ...
  ctx.effect(
    ...
    'dsh-yorha-ui: YoRHa alias-token layer'
  );
  ...
  ctx.slots.inject('conversation.hero.brand.mark', () =>
    ctx.slots.register(
      { name: 'conversation.hero.brand.mark', id: 'dsh-yorha-ui' },
      (props?: BrandMarkProps) => {
        const { size = 34, className } = props ?? {};
        const elementProps: ReactProps = {
        ...
        return createElement('span', elementProps);
      }
    )
  );
  ...
  ctx.slots.inject('sidebar.brand.mark', () =>
    ctx.slots.register(
      { name: 'sidebar.brand.mark', id: 'dsh-yorha-ui' },
      (props?: BrandMarkProps) => {
